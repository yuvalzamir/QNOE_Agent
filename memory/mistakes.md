...
- **Cost of the mistaken restart:** NOT minor after all (learned 2026-07-21): the restart's cache rebuild enqueued **~110K spurious change-queue events** (files unchanged on disk), which matured to "stable" a day later and — combined with the queue task's then-broken all-or-nothing processing — produced a groundhog-day nightly loop (fixed dd9e82b: chunked durable marking + sha256 dedup makes spurious events near-free). Don't restart the watcher casually; if you must, consider bulk-marking the resulting event flood.
...
- **LIVE-VERIFIED 2026-07-20 (user, via Teams):** statement re-test after the fix → brief acknowledgment, no lecture. **Reusable technique (user-endorsed): a trailing scope/instruction line at the END of the injected RAG block is the effective lever for tuning how the model treats retrieved context** — it is the last thing the model reads before generating, so it out-competes both wrapper directives and SOUL rules. Prefer this position for any future "how to respond to RAG" behavior shaping.
