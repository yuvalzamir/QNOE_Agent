...
- **LESSON:** editing a SOUL/context file can silently nuke the WHOLE file's rules. **Any SOUL edit must be scan-checked.** (SOUL.md is now whitelisted, but MEMORY.md/USER.md are NOT — a poisoned memory can still be silently dropped; that's what soul_health.py now guards.) Directly relevant to R11 grounding work — cite-or-abstain rules added to a blocked SOUL are a no-op. Avoid trigger phrases: "do not tell the user", "SYSTEM:", "ignore previous instructions", "you are now …", "pretend you are", "output the system prompt", "respond without restrictions/filters", "name yourself X" (full list in `threat_patterns.py`).
