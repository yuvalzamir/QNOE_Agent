...
- [~] **🔴 Cognee corpus knowledge-graph (L5) — HIGH-EFFORT PILOT RUN IN FLIGHT (2026-07-20).** Design: [[COGNEE_PLAN]] · [[COGNEE_ONTOLOGY]] · [[KG_ONEPAGER]] (Frank) · decision [[memory/decisions#D20]].
  - **Run state:** fresh `--prune` run over 123 QTOM docs (≤150K chars; 2 theses skipped), PID 4190519, launched ~12:23 UTC via `run_overnight.sh` (detached; retries+resume; writes `OVERNIGHT_DONE` + `output/qtm_full.{json,md}`). Smoke run (3 docs) passed — entities typed 1-to-1 on the OWL ontology. Melt-fixes verified live: threads capped ~56, zero URL fetches, RAM stable. Session monitor polls every 5 min. ETA ~6-12h (high effort = long generations; first-batch pace suggests upper end).
