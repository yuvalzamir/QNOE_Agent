...
- [~] **🔴 Cognee corpus knowledge-graph (L5) — HIGH-EFFORT PILOT RUN IN FLIGHT (2026-07-20).** Design: [[COGNEE_PLAN]] · [[COGNEE_ONTOLOGY]] · [[KG_ONEPAGER]] (Frank) · decision [[memory/decisions#D20]].
  - **Run state (HIGH run #2, relaunched ~13:50 UTC):** run #1 was killed after 50 min with ZERO results — **litellm's 600s default client timeout starved every call** (high effort = 10-11K reasoning tokens ≈ 9-10+ min/call under 4-slot concurrency → client aborts + retries while llama generates into the void; diagnosed via cognee_db: 50 server-side completions, 0 `session_model_usage`/`nodes` rows). **Fix b05aecb:** explicit `llm_args={"timeout": 3600}` (`LLM_CLIENT_TIMEOUT` env) — deployed; run #2 log confirms `llm client timeout -> 3600s`. Fresh `--prune`, 123 docs, PID 65553; monitor now also tracks the decisive metric (`llm_calls_ok`/`nodes` from cognee_db) and will emit "TIMEOUT FIX CONFIRMED" on the first successful call. **USER CHECKPOINT ~2h in (~15:50 UTC): review throughput → continue at high (~24-26h total) or downshift.** Earlier melt-fixes still verified (threads capped, zero URL fetches). Starved-run log: `overnight.log.jul20-starved`.
