...
- [~] **I4 — SharePoint access + embedding** — LIVE (2026-07-03). Two sites indexed: `twisted-materials` (QTOM, SpectroMag, THz gas laser) + `noe-group` (all). Delta sync every 30 min via `SharePointPoller`. Nightly full sync as safety net. ONGOING: monitor delta sync health, verify nightly runs, expand site/folder coverage as needed.
  ...
  - [x] **DESIGN FIX (from M47) — stop silent drops in `delta_sync`** ✅ *(2026-07-13, branch `feature/sp-ingest-hardening`, deployed)* — added `sp_retry_queue`: `_process_item` raises on retryable failures, `delta_sync` parks them and re-attempts at the top of each cycle (bounded `SP_RETRY_MAX_ATTEMPTS=5`). Token still advances; failures no longer lost.
  - [x] **DESIGN FIX (from M47) — `ProcessPoolExecutor` crash on 2nd Docling conversion** ✅ *(2026-07-13, deployed)* — `_chunk_file_safe` now uses a **forkserver** pool forked from a clean pre-torch server (`_ensure_chunk_server`), not a fork of the torch-polluted main process. Validated: 2 PDFs chunk in one process with torch loaded. Not spawn (would re-import the tree per file → cripples full_sync).
